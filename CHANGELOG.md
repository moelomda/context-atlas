...
- Git history file-status extraction now uses bounded NUL-safe `diff-tree --stdin` batches instead of one subprocess per commit, with overflow splitting, semantic-reference and unusual-path regression tests.
