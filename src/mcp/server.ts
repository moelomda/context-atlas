...
const repoSchema = z
  ...
  .max(4_096)
  .optional()
  .describe("Path inside an initialized Context Atlas Git repository. Defaults to the current directory.");
...
server.registerTool(
  "atlas_overview",
  {
    ...
    description:
      "Return the current evidence-backed project summary, statistics, risks, and recent history. Pending proposals are excluded.",
...
server.registerTool(
  "atlas_context_pack",
  {
    ...
    description:
      "Build a bounded, task-specific context pack with confidence labels, health warnings, and evidence citations before a coding change.",
    inputSchema: {
      task: z.string().min(1).max(2_000),
      tokenBudget: z.number().int().min(500).max(20_000).optional(),
      overrideId: z
        .string()
        .regex(/^pack_override_[a-f0-9]{24}$/)
        .optional()
...
const packSnapshotIdSchema = z
  .string()
  .regex(/^pack_snapshot_[a-f0-9]{64}$/)
...
server.registerTool(
  "atlas_pack_snapshot",
  {
    ...
    description:
      "Verify and read one immutable context-pack snapshot. By default this returns safe metadata; request includePack only when the full historical pack is necessary.",
    inputSchema: {
...
server.registerTool(
  "atlas_pack_diff",
  {
    ...
    description:
      "Compare two verified immutable context-pack snapshots across task, repository, freshness, policy, sections, selected claims, relationships, events, evidence, and warnings.",
    inputSchema: {
...
server.registerTool(
  "atlas_explain",
  {
    ...
    description:
      "Explain a component, document, decision, dependency, or project entity with versions, relationships, history, and evidence.",
...
server.registerTool(
  "atlas_health",
  {
    ...
    description:
      "Check freshness, evidence coverage, ledger integrity, proposal conflicts, secret containment, and repository synchronization.",
    inputSchema: { repo: repoSchema },
    annotations: readAnnotations,
  },
...
server.registerTool(
  "atlas_evidence",
  {
    ...
    description:
      "Resolve one evidence identifier to its safe locator, digest, observation time, and sensitivity label. Sensitive locators and host-specific metadata remain withheld.",
    inputSchema: {
...
server.registerTool(
  "atlas_assertions",
  {
    ...
    description:
      "Return evidence-linked temporal assertions. Every row includes a mandatory presentation status, settled flag, reason, evidence, and current/as-of scope; immutable lifecycle alone must never be interpreted as current authority.",
    inputSchema: {
...
function boundedMcpResult<T extends { content: Array<{ type: "text"; text: string }>; structuredContent: Record<string, unknown> }>(
  response: T,
): T {
...
function contextPackResult(root: string, task: string, tokenBudget?: number, overrideId?: string) {
  ...
  for (let pass = 0; pass < 4; pass += 1) {
    ...
    for (let metadataPass = 0; metadataPass < 8; metadataPass += 1) {
      ...
      if (envelope.transport.serializedCharacters === serializedCharacters && envelope.transport.estimatedTokens === estimatedTokens) break;
