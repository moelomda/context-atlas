...
import { getCanonicalProjectEntity, isCanonicalProjectOverviewAssertion, projectOverviewClaimProjection } from "./claim-status.js";
...
export function getHealthReport(repoRoot: string, database?: AtlasDatabase, knownRepository?: RepoStatus): HealthReport {
  ...
  checks.push(
    check(
      ...
      ledgerMatches
        ? "Commit the ledger with the project so future changes remain reviewable."
        : "Stop context updates, preserve both files, and investigate tampering or an interrupted write.",
    ),
  );
  ...
  const invalidEventAnchorPreview = invalidEventAnchors
    .slice(0, 8)
  ...
  checks.push(
    check(
      ...
      invalidEventAnchors.length === 0
        ? "No action required."
        : "Stop synchronization; preserve the database and ledger, then recover from a verified backup or investigate an interrupted/tampered write.",
    ),
  );
  ...
  checks.push(
    check(
      ...
      0,
      repository.dirty
        ? `${repository.changedFiles} uncommitted path${repository.changedFiles === 1 ? " is" : "s are"} outside the immutable Git history.`
        : "Working tree is clean.",
      repository.dirty
        ? "Commit or explicitly describe unfinished work before handing the project to another person or model."
        : "No action required.",
    ),
  );
  ...
  checks.push(
    check(
      ...
      stale.length === 0 ? 0 : 2,
      stale.length === 0
        ? "All active items are within their freshness window."
        : `${stale.length} active item${stale.length === 1 ? " is" : "s are"} older than the configured freshness window.`,
  ...
  checks.push(
    check(
      ...
      missingEvidence === 0
        ? "Every active entity has primary evidence."
        : `${missingEvidence} active item${missingEvidence === 1 ? " has" : "s have"} no valid primary evidence.`,
      missingEvidence === 0
        ? "Keep evidence attached to every accepted claim."
        : "Do not present unsupported items as project truth; attach evidence or supersede them.",
    ),
  );

  const currentAssertionEvidence = db.db
    .prepare(`
  ...
  const currentEvidenceIds = [
    ...new Set([
      ...
      ...db
        .listRelationships()
        .filter((relationship) => relationship.active && relationship.evidenceId)
  ...
  const assertionIntegrity = db.db
    .prepare(`
  ...
  checks.push(
    check(
      ...
      invalidAssertions === 0
        ? "No action required."
        : "Stop authoritative projection and repair or import a verified canonical assertion history.",
    ),
  );

  const currentAcceptedAssertionMetadata = db.db
    .prepare(`
  ...
  const invalidGuidanceBoundaries = currentAcceptedAssertionMetadata.filter((row) => {
    ...
    return (
      typeof metadata.reviewedGuidanceWatermark !== "string" ||
  ...
  checks.push(
    check(
      ...
      invalidGuidanceBoundaries.length === 0
        ...
        : `${invalidGuidanceBoundaries.length} current accepted assertion${invalidGuidanceBoundaries.length === 1 ? " is" : "s are"} missing or mismatched against the current extraction-policy watermark: ${invalidGuidanceBoundaries
            .slice(0, 8)
            .map((row) => row.id)
            .join(", ")}${invalidGuidanceBoundaries.length > 8 ? `, plus ${invalidGuidanceBoundaries.length - 8} more` : ""}.`,
      invalidGuidanceBoundaries.length === 0
        ? "No action required."
  ...
  checks.push(
    check(
      ...
      assertionConflicts.length === 0
        ? "No action required."
        : "Preserve both claims, inspect their evidence, and record an explicit temporal or scope resolution.",
    ),
  );

  const overrideRows = db.db
    .prepare("SELECT id, actor, reason, reason_digest, critical_digest, created_at, expires_at FROM context_pack_overrides")
    .all() as Array<Record<string, unknown>>;
  const invalidOverrides = overrideRows.filter(
    (row) =>
      !/^pack_override_[a-f0-9]{24}$/.test(String(row.id)) ||
  ...
  checks.push(
    check(
      ...
      invalidOverrides.length === 0
        ? "No action required."
        : "Do not use overrides; preserve the database and investigate corruption or tampering.",
    ),
  );
  ...
  checks.push(
    check(
      ...
      sensitiveEvidence === 0
        ? "No potential secrets were encountered."
        : `${sensitiveEvidence} evidence record${sensitiveEvidence === 1 ? " was" : "s were"} flagged; raw content was not stored.`,
      sensitiveEvidence === 0
        ? "Continue excluding credentials and local environment files."
        : "Inspect the source repository, rotate exposed credentials if necessary, and keep flagged content withheld.",
    ),
  );
  ...
  checks.push(
    check(
      ...
      conflictGroups.size === 0
        ? "No proposal conflicts are waiting for resolution."
        : `${conflictGroups.size} conflicting proposal group${conflictGroups.size === 1 ? " requires" : "s require"} human resolution.`,
  ...
  checks.push(
    check(
      ...
      truncated ? 2 : 0,
      truncated
        ? "The configured file limit truncated the repository scan."
        : "The repository scan completed within its configured file limit.",
