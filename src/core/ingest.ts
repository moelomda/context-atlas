...
import type { EntityRecord, EvidenceRecord, ProposalRecord, RelationshipRecord, SyncResult } from "./types.js";
...
export function syncRepository(start = process.cwd()): SyncResult {
  ...
  if (!ledgerVerification.consistent || ledgerVerification.unflushedEntries > 0 || database.countUnledgeredEvents() > 0) {
    database.close();
    throw new Error(
      "Context history integrity check failed. Run `context-atlas health` and recover from a verified backup before synchronizing.",
    );
  }
  const eventIntegrity = getHealthReport(root, database, repository).checks.find((item) => item.id === "event-ledger-coverage");
  if (!eventIntegrity || eventIntegrity.status === "critical") {
    database.close();
    throw new Error(
      `Timeline event integrity check failed before synchronization: ${eventIntegrity?.details ?? "the event integrity check was unavailable"}`,
    );
  }
    ...
    const syncResult = database.transaction(() => {
      ...
      for (const commit of commits) {
        ...
        const commitPaths = commit.files.map((file) => ({
          ...
          ...(file.previousPath ? { previousPath: presentCommitPath(file.previousPath, scanExclusions, atlasIgnore.matches) } : {}),
        }));
        const evidence = makeEvidence(
          ...
          sha256(commit.hash),
          startedAt,
          cleanSubject.sensitive || cleanAuthor.sensitive,
          {
            commit: commit.hash,
        ...
        const inserted = database.insertEvent({
          ...
          summary: summarizeCommit(
            commit.hash,
            commitPaths.map((file) => file.path),
          ),
          commit: commit.hash,
          files: commitPaths,
      ...
      const existingRepositoryProject = database
        .listEntities({ types: ["project"], includeRemoved: true })
      ...
      for (const relativePath of safeFiles.filter(isContextDocumentPath)) {
        ...
        database.upsertEntity(
          ...
          [evidence.id],
          decision ? "decision document scan" : "documentation scan",
        );
        database.upsertRelationship(
          makeRelationship(
            nearestComponent(relativePath, componentIds) ?? projectId,
            documentId,
            decision ? "governed_by" : "documented_by",
            evidence.id,
          ),
        );
        documentsObserved += 1;
      ...
      const reviewedBoundaryChanged = Boolean(
        approvedNarrative && (synchronizedHeadChanged || synchronizedWorktreeChanged || guidanceDependenciesChanged || commitsAdded > 0),
      );
      if (reviewedBoundaryChanged && approvedNarrative) {
        ...
        const staleAssertion =
          previousAssertion && (previousAssertion.lifecycle === "accepted" || previousAssertion.lifecycle === "stale")
        ...
        database.upsertEntity(
          ...
          supportingEvidence,
          "repository changed after narrative approval",
        );
      }
      ...
      if (commitEvidence.length > 0 || reviewedBoundaryChanged) {
        ...
        !database
          ...
          .some(
            (proposal) =>
              proposal.kind === "context_update" && proposal.payload.observedGuidanceWatermark === guidanceDependencies.watermark,
          )
      ) {
        const proposal = createContextProposal(
          projectId,
...
function collectComponents(root: string, files: string[], maxDepth: number): Map<string, ComponentAccumulator> {
  ...
  for (const relativePath of files) {
    ...
    let bytes = 0;
    try {
      bytes = statSync(assertInside(root, relativePath)).size;
    } catch {
    ...
    try {
      ...
      const dependencies =
        parsed.dependencies && typeof parsed.dependencies === "object"
          ...
          : [];
      const developmentDependencyCount =
...
function isContextDocumentPath(relativePath: string): boolean {
  ...
  return (
    /^(readme|project|architecture|contributing|roadmap)(\.[a-z0-9_-]+)?\.md$/.test(base) ||
...
function extractDocumentSummary(relativePath: string, markdown: string): { title: string; summary: string } {
  const title =
    markdown
      .match(/^#{1,2}\s+(.+)$/m)?.[1]
      ?.replace(/[*_`]/g, "")
      .trim() || path.posix.basename(relativePath);
  ...
  return {
    title: sanitizeText(title, 300).value,
    summary: sanitizeText(paragraph.join(" ") || `Documentation at ${relativePath}.`, 700).value,
  };
}
