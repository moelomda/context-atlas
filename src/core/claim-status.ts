...
export function projectOverviewClaimProjection(
  ...
  const canonicalAssertion = assertion && isCanonicalProjectOverviewAssertion(assertion, canonicalProjectId)
    ? assertion
    : undefined;
  ...
  const evidence = canonicalAssertion?.evidence ?? (narrative?.primaryEvidenceId
    ? [{ evidenceId: narrative.primaryEvidenceId, role: "support" as const }]
    : []);
  ...
  const hasSupportingEvidence = supportingEvidenceIds.length > 0
    && supportingEvidenceIds.every((id) => !unusableEvidenceIds.has(id));
  ...
  if (canonicalAssertion) {
    if (canonicalAssertion.lifecycle === "conflicting" || conflictingAssertionIds.has(canonicalAssertion.id)) {
      status = "conflicting";
      reason = metadataReason(canonicalAssertion)
      ...
      status = "stale";
      reason = synchronizedWorkingTreeFingerprint === null
...
export function getPresentedAssertion(repoRoot: string, assertionId: string): PresentedAssertion | null {
  ...
  return current?.id === assertion.id
    ? current
    : { ...assertion, presentation: historicalPresentation(assertion, "history") };
}
...
function lifecyclePresentation(
  ...
  const status: AssertionPresentationStatus = conflictingAssertionIds.has(assertion.id)
    || assertion.lifecycle === "conflicting"
      ...
      : status === "unknown" && !reviewedGuidanceWatermark
        ...
        : status === "unknown" && assertion.authority === "inferred"
          ...
          : status === "unknown" && unsupported
            ...
            : metadataReason(assertion) ?? `The selected revision has lifecycle '${assertion.lifecycle}' and is not settled current guidance.`;
  return {
    status,
    settled: status === "current",
    reason,
