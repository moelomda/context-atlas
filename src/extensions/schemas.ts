...
const artifactFragmentSchema = z
  .object({
...
const coverageDiagnosticSchema = z
  .object({
    code: nameSchema,
...
const approvedArtifactSchema = z
  .object({
...
const evidenceExtractorCapabilitiesSchema = z
  .object({
...
const languageAnalyzerCapabilitiesSchema = z
  .object({
...
const inferenceProviderCapabilitiesSchema = z
  .object({
...
const redactorCapabilitiesSchema = z
  .object({
    categories: z.array(nameSchema).min(1).max(128),
    actions: z
      .array(z.enum(["redact", "block"]))
      .min(1)
      .max(2),
...
const exporterCapabilitiesSchema = z
  .object({
...
const validatorCapabilitiesSchema = z
  .object({
...
const evidenceExtractorInputSchema = z
  .object({
    schemaVersion: z.literal(EXTENSION_SCHEMA_VERSION),
...
const extractedObservationSchema = z
  .object({
...
const evidenceExtractorOutputSchema = z
  .object({
    schemaVersion: z.literal(EXTENSION_SCHEMA_VERSION),
...
const languageAnalyzerInputSchema = z
  .object({
    schemaVersion: z.literal(EXTENSION_SCHEMA_VERSION),
    artifact: approvedArtifactSchema,
    language: nameSchema,
...
const languageAnalyzerOutputSchema = z
  .object({
    schemaVersion: z.literal(EXTENSION_SCHEMA_VERSION),
...
const inferenceProviderInputSchema = z
  .object({
    schemaVersion: z.literal(EXTENSION_SCHEMA_VERSION),
    ...
    credentialReference: z
      ...
      .min(1)
      .max(500)
      .regex(/^(?:env|os-keychain|credential-store):[A-Za-z_][A-Za-z0-9_.:-]{0,255}$/)
      .nullable(),
    maxOutputTokens: z.number().int().min(1).max(1_000_000),
...
const inferenceProviderOutputSchema = z
  .object({
    schemaVersion: z.literal(EXTENSION_SCHEMA_VERSION),
...
const canonicalExportDtoSchema = z
  .object({
    schemaVersion: z.literal(EXTENSION_SCHEMA_VERSION),
...
const validatorSubjectSchema = z
  .object({
    subjectId: identifierSchema,
    kind: nameSchema,
    payload: jsonValueSchema,
...
export function validateExtensionOutputSemantics<K extends ExtensionKind>(
  ...
  switch (kind) {
    ...
    case "inference-provider":
      ...
      break;
    case "redactor":
      validateRedactorOutput(input as RedactorInputV1, output as RedactorOutputV1, manifest as ExtensionManifestFor<"redactor">);
      break;
    case "exporter":
      validateExporterOutput(input as ExporterInputV1, output as ExporterOutputV1, manifest as ExtensionManifestFor<"exporter">);
      break;
    case "validator":
      validateValidatorOutput(input as ValidatorInputV1, output as ValidatorOutputV1, manifest as ExtensionManifestFor<"validator">);
      break;
  }
...
function validateEvidenceExtractorInput(input: EvidenceExtractorInputV1, manifest: ExtensionManifestFor<"evidence-extractor">): void {
  ...
  if (
    !manifest.capabilities.artifactKinds.includes(input.artifact.kind) ||
    !manifest.capabilities.mediaTypes.includes(input.artifact.mediaType) ||
    input.policy.maxObservations > manifest.capabilities.maxObservations
  )
    fail();
...
function validateEvidenceExtractorOutput(
  ...
  for (const observation of output.observations) {
    if (
      !input.policy.allowedEntityTypes.includes(observation.subject.entityType) ||
      !input.policy.allowedPredicates.includes(observation.predicate)
    )
      fail();
    validateFragment(observation.fragment, input.artifact, artifactBytes);
    if (
      !observation.dependencies.some(
        (dependency) =>
          dependency.kind === "artifact" && dependency.id === input.artifact.artifactId && dependency.digest === input.artifact.sha256,
      )
    )
      fail();
...
function validateLanguageAnalyzerInput(input: LanguageAnalyzerInputV1, manifest: ExtensionManifestFor<"language-analyzer">): void {
  validateApprovedArtifact(input.artifact, manifest.capabilities.maxArtifactBytes);
  if (
    !manifest.capabilities.languages.includes(input.language) ||
    !manifest.capabilities.mediaTypes.includes(input.artifact.mediaType) ||
    input.policy.maxStructures > manifest.capabilities.maxStructures
  )
    fail();
}
...
function validateInferenceProviderInput(input: InferenceProviderInputV1, manifest: ExtensionManifestFor<"inference-provider">): void {
  if (
    input.providerId !== manifest.id ||
    !manifest.capabilities.purposes.includes(input.purpose) ||
    !manifest.capabilities.models.includes(input.model) ||
    input.maxOutputTokens > manifest.capabilities.maxOutputTokens
  )
    fail();
...
function validateInferenceProviderOutput(
  ...
  if (
    output.modelVersion !== input.model ||
    ...
    (output.usage.costMicros === null) !== (output.usage.currency === null)
  )
    fail();
  ...
  for (const candidate of output.candidates) {
    ...
    if (
      !allowedSubjects.has(candidate.subjectId) ||
      !allowedPredicates.has(candidate.predicate) ||
      candidate.contradictingEvidenceIds.some((evidenceId) => support.has(evidenceId)) ||
      [...candidate.supportingEvidenceIds, ...candidate.contradictingEvidenceIds].some((evidenceId) => !allowed.has(evidenceId))
    )
      fail();
...
function validateRedactorInput(input: RedactorInputV1, manifest: ExtensionManifestFor<"redactor">): void {
  if (
    input.items.length > manifest.capabilities.maxItems ||
    input.maxSpans > manifest.capabilities.maxSpans ||
    input.allowedCategories.some((category) => !manifest.capabilities.categories.includes(category))
  )
    fail();
...
function validateRedactorOutput(input: RedactorInputV1, output: RedactorOutputV1, manifest: ExtensionManifestFor<"redactor">): void {
  ...
  for (const item of output.items) {
    ...
    for (const span of item.spans) {
      ...
      if (
        ...
        !manifest.capabilities.actions.includes(span.action)
      )
        fail();
      previousEnd = span.endByte;
    }
    const expectedAction = item.spans.some((span) => span.action === "block") ? "block" : item.spans.length > 0 ? "redact" : "allow";
...
function validateExporterInput(input: ExporterInputV1, manifest: ExtensionManifestFor<"exporter">): void {
  if (!manifest.capabilities.formats.includes(input.requestedFormat) || input.maxOutputBytes > manifest.capabilities.maxTotalBytes) fail();
...
function validateExporterOutput(input: ExporterInputV1, output: ExporterOutputV1, manifest: ExtensionManifestFor<"exporter">): void {
...
function validateValidatorInput(input: ValidatorInputV1, manifest: ExtensionManifestFor<"validator">): void {
  if (!manifest.capabilities.scopes.includes(input.scope) || input.maxFindings > manifest.capabilities.maxFindings) fail();
  assertUnique(input.allowedEvidenceIds);
...
function validateValidatorOutput(input: ValidatorInputV1, output: ValidatorOutputV1, manifest: ExtensionManifestFor<"validator">): void {
  ...
  for (const finding of output.findings) {
    if (
      !manifest.capabilities.ruleIds.includes(finding.ruleId) ||
...
function validateFragment(fragment: ArtifactFragmentV1, artifact: ApprovedArtifactV1, bytes: Buffer): void {
  ...
  if (
    fragment.artifactId !== artifact.artifactId ||
    ...
    (requiresUtf8Boundary && !isUtf8CodePointBoundary(bytes, fragment.endByte))
  )
    fail();
}
...
function validateCoverage(coverage: { bytesExamined: number; bytesCovered: number; complete: boolean }, totalBytes: number): void {
  if (
    coverage.bytesExamined > totalBytes ||
    coverage.bytesCovered > coverage.bytesExamined ||
    (coverage.complete && coverage.bytesExamined !== totalBytes)
  )
    fail();
}
...
function isUtf8CodePointBoundary(bytes: Buffer, offset: number): boolean {
  return offset === 0 || offset === bytes.byteLength || (offset > 0 && offset < bytes.byteLength && ((bytes[offset] ?? 0) & 0xc0) !== 0x80);
}
...
function isReservedAuthorityToken(value: string): boolean {
  ...
  return (
    RESERVED_AUTHORITY_TOKENS.has(normalized) ||
    /^(?:is)?(?:acceptance|accepted|approval|approved|authority|lifecycle|review|reviewed|reviewer)(?:by|id|level|state|status)?$/.test(
      normalized,
    )
  );
}
